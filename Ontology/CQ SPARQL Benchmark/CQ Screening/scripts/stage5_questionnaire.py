...
def write_apps_script(items, per_section=6):
    """Emit a Google Apps Script that builds the same questionnaire in Forms.

    Generated from the same ITEMS table as the Markdown form, so the two
    cannot drift apart. Item numbering matches cq_stage5_items.csv, which is
    how responses are joined back to the key.

    One deliberate difference from the printable version: the five numbered
    slots for missing questions become a single free-text field, because five
    empty boxes on a screen read as an obligation rather than an invitation.
    ...
    L = [
        ...
        " * execution log. Item order is fixed (seed " + str(SHUFFLE_SEED) + ")",
        ...
        "",
        "var TITLE = 'Kuesioner Validasi Pakar - Pertanyaan Kompetensi "
        "Knowledge Graph Hama dan Penyakit Padi';",
        "",
        "var INTRO = [",
        "  'Kami sedang MERANCANG sebuah basis pengetahuan (knowledge graph) "
        "tentang hama dan penyakit padi, yang akan menggabungkan pengetahuan "
        "dari literatur dengan citra gejala di lapangan.',",
        "  '',",
        ...
        "ditetapkan SEBELUM sistemnya dibangun.',",
        "  '',",
        "  'Belum ada sistem yang perlu Anda coba, dan Anda tidak perlu "
        "menjawab pertanyaan-pertanyaannya. Yang kami minta adalah penilaian "
        "Anda sebagai pakar atas pertanyaannya sendiri.',",
        "  '',",
        "  'Pengisian diperkirakan memakan waktu 20-30 menit.'",
        "].join('\\n');",
        "",
        "var LEGEND = [",
        "  'Relevansi - seberapa penting pertanyaan ini bagi praktik diagnosis "
        "dan pengelolaan hama serta penyakit padi?  (1 = tidak relevan, "
        "5 = sangat penting)',",
        "  'Kejelasan - apakah pertanyaan ini dirumuskan dengan jelas dan tidak "
        "menimbulkan tafsir ganda?  (1 = sangat kabur, 5 = sangat jelas)'",
        "].join('\\n');",
        "",
    ...
    for it in items:
        L.append("  [%d, %s,\n   %s]," % (it["item_no"],
                                          js_string(it["question_id"]),
                                          js_string(it["question_en"])))
    L += [
        "];",
        "",
        ...
        "    if (i %% %d === 0) {" % per_section,
        "      var page = form.addPageBreakItem()",
        "        .setTitle('Pertanyaan ' + (i + 1) + '-' +",
        "                  Math.min(i + %d, ITEMS.length));" % per_section,
        ...
        "      .setHelpText('Mohon diisi terutama bila Anda memberi nilai "
        "rendah, atau bila istilah yang kami gunakan keliru menurut praktik "
        "di lapangan.')",
        "      .setRequired(false);",
        "  }",
        "",
        ...
        "    .setHelpText('Bagian ini sama pentingnya dengan penilaian di "
        "atas. Daftar tersebut kami susun dari sumber otomatis, sehingga besar "
        "kemungkinan ada hal yang penting di lapangan namun tidak muncul di "
        "sana.');",
        "  form.addParagraphTextItem()",
        "    .setTitle('Menurut Anda, pertanyaan apa yang seharusnya masuk "
        "dalam cakupan basis pengetahuan ini, tetapi belum ada dalam daftar "
        "di atas?')",
        "    .setHelpText('Tuliskan satu pertanyaan per baris. Boleh lebih "
        "dari satu, boleh juga dikosongkan.')",
        "    .setRequired(false);",
        "",
    ...
    (REPORTS / "cq_stage5_google_form.js").write_text("\n".join(L),
                                                      encoding="utf-8")
