...
| `Biological_Control` | Treatment | [`biological control`](http://aims.fao.org/aos/agrovoc/c_918) | ~~`skos:exactMatch`~~ `skos:closeMatch` | Implemented in v2.5; downgraded 2026-09-15 | Direct label match. Downgraded when `Biological_Control_Category` (round 6) took `exactMatch` on the same concept: c_918 is the control-method category, while this individual is the practice applied against a pest. |
...
| `Biological_Control` (Treatment) | `skos:exactMatch` → `skos:closeMatch` | Muhammad Ariful Furqon | Practice vs category, after `Biological_Control_Category` took c_918 | 2026-09-15 |
